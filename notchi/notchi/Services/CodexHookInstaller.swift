...
struct CodexConfigDirectoryResolution {
    ...
    var hookScriptURL: URL {
        hooksDirectoryURL.appendingPathComponent("notchi-codex-hook.sh")
    }
...
struct CodexHookInstaller {
    ...
    static let hookCommand = "\"${CODEX_HOME:-$HOME/.codex}/hooks/notchi-codex-hook.sh\""
    static var testHooks = makeDefaultTestHooks()
    ...
    static func uninstall() {
        ...
        codexLogger.info("Uninstalled Notchi Codex hooks")
    }
    ...
    private static func updateHooksJSON(at hooksURL: URL, command: String) -> Bool {
        ...
        do {
            ...
            codexLogger.info("Updated hooks.json with Notchi Codex hooks")
            return true
        } catch {
    ...
    private static func upsertEventHook(
        ...
        for index in existingEvent.indices {
            ...
            entryHooks.removeAll { hook in
                ...
                let isLegacy = cmd.contains("notchi-codex-hook.sh") && cmd != command
    ...
    private static func makeDefaultTestHooks() -> TestHooks {
        TestHooks(
            bundledHookScriptURL: {
                Bundle.main.url(forResource: "notchi-codex-hook", withExtension: "sh")
            }
        )
    }
}
