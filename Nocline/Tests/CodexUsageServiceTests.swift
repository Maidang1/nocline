...
final class CodexUsageServiceTests: XCTestCase {
    ...
    func testRefreshReturnsUnavailableWhenAPIFailsAndNoCacheExists() async {
        ...
        XCTAssertEqual(service.state, .unavailable(.networkError))
    }
    ...
    func testSectionPresentationShowsUnavailableRows() {
        let presentation = CodexUsageSectionPresentation.make(from: .unavailable(.networkError))

        XCTAssertEqual(presentation.statusText, "Unavailable")
        XCTAssertEqual(presentation.summaryMessage, "Check internet connection")
        XCTAssertEqual(presentation.rows.map(\.detailText), ["Check your internet connection", "Check network connection"])
    }
